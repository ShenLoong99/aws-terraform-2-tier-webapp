...
<head>
    ...
    <style>
        .completed-task { text-decoration: line-through; color: #adb5bd; font-style: italic; }
        .card { border-radius: 15px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); border:none; }
        .task-row { transition: all 0.2s; }
        .task-row:hover { background-color: #f8f9fa; }
    </style>
</head>
<body class="bg-light">
    <nav class="navbar navbar-dark bg-dark mb-4 p-3"><div class="container"><span class="navbar-brand fw-bold">🚀 To Do List</span></div></nav>
    <div class="container"><div class="row justify-content-center"><div class="col-md-8">
        <div class="card p-4">
            ...
            <div class="input-group mb-4">
                ...
                <button onclick="addTask()" class="btn btn-primary px-4"><i class="bi bi-plus-lg"></i></button>
            </div>
            <div id="taskList"><p class="text-center text-muted">Loading tasks...</p></div>
        </div>
        <p class="text-center text-muted mt-3 small">Server ID: $${os.hostname()}</p>
    </div></div></div>
    <script>
        async function loadTasks() {
            try {
                ...
                if (tasks.length === 0) {
                    list.innerHTML = '<p class="text-center text-muted py-3">No tasks yet!</p>';
                    return;
                }
        ...
        async function doneTask(id) {
            await fetch(\`/api/tasks/complete/\\$${id}\`, { method: 'PUT' });
            loadTasks();
        }
